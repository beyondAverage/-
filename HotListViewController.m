...
- (void)viewDidLoad {
    ...
    [self customNavigationTitle:@"热榜" andSize:22 andColor:[UIColor blackColor]];
}
...
{
    NSString *urlString = [NSString stringWithFormat:@"http://open.candou.com/mobile/hot/page/%ld",page];
...
{
    HotListTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:kReuseIdentifier];
    if (!cell) {
        cell = [[HotListTableViewCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:kReuseIdentifier];
    }
...
- (void)updateSearchResultsForSearchController:(UISearchController *)searchController
{
    ...
    for (int i=0; i<_dataList.count; i++) {
        HotListModel *model = [_dataList objectAtIndex:i];
        //根据名字查找
        NSString *name = [model.hotListDict objectForKey:@"name"];
        NSString *shares = [model.hotListDict objectForKey:@"shares"];
