...
- (void)viewDidLoad {
    ...
    [self customNavigationTitle:@"免费" andSize:22 andColor:[UIColor purpleColor]];
}
...
{
    NSString *urlString = [NSString stringWithFormat:@"http://iappfree.candou.com:8080/free/applications/free?currency=rmb&page=%ld",page];
...
{
    FreeTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:kReuseIdentifier];
    if (!cell) {
        cell = [[FreeTableViewCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:kReuseIdentifier];
    }
...
- (void)updateSearchResultsForSearchController:(UISearchController *)searchController
{
    ...
    for (int i=0; i<_dataList.count; i++) {
        FreeModel *model = [_dataList objectAtIndex:i];
        //根据名字查找
        NSString *name = [model.freeDict objectForKey:@"name"];
        NSString *shares = [model.freeDict objectForKey:@"shares"];
