...
- (void)showDepreciateCell:(HotListModel *)cellMode
{
    ...
    //利用url异步加载图片
    [_appImageView setImageWithURL:[NSURL URLWithString:[cellMode.hotListDict objectForKey:@"iconUrl"]]];
    
    _nameLabel.text = [cellMode.hotListDict objectForKey:@"name"];
    ...
    //剪裁前景图片
    float starCurrent = [[cellMode.hotListDict objectForKey:@"starCurrent"] floatValue];
    if (starCurrent) {
    ...
    _starOverallLabel.text = [NSString stringWithFormat:@"评分: %@分",[cellMode.hotListDict objectForKey:@"starOverall"]];
    _starOverallLabel.adjustsFontSizeToFitWidth = YES;
    
    
    _sharesLabel.text = [NSString stringWithFormat:@"分享: %@次",[cellMode.hotListDict objectForKey:@"shares"]];
    _sharesLabel.adjustsFontSizeToFitWidth = YES;
    
    
    _favoritesLabel.text = [NSString stringWithFormat:@"收藏: %@次",[cellMode.hotListDict objectForKey:@"favorites"]];
    _favoritesLabel.adjustsFontSizeToFitWidth = YES;
    
    _downloadsLabel.text = [NSString stringWithFormat:@"下载: %@次",[cellMode.hotListDict objectForKey:@"downloads"]];
